...
def convert_csv_to_sru(input_file, output_file, identity_number, name, 
                       date=None, time=None):
    """
...
@click.option('--identity', required=True, help='The identity number to use in the SRU file.')
...
def cli(input_file, output_file, identity, name):
    ...
    with open(input_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
        convert_csv_to_sru(infile, outfile, identity, name, date=current_date, time=current_time)
